...
pub struct QueryClient {
    ...
    pub(crate) cache: Rc<RefCell<HashMap<(TypeId, TypeId, TypeId), Box<dyn CacheEntryTrait>>>>,
}

pub(crate) struct CacheEntry<K: 'static, V: 'static, E: 'static>(HashMap<K, Query<K, V, E>>);
...
impl QueryClient {
    ...
    //     V: Clone + 'static,
    // {
    //     self.use_cache_option(|cache: &HashMap<K, Query<K, V, E>>| {
    //         cache
    ...
    fn use_cache_option<K, V, E, F, R>(&self, func: F) -> Option<R>
    where
        K: 'static,
        V: 'static,
        E: 'static,
        ...
        R: 'static,
    {
        ...
        let type_key = (TypeId::of::<K>(), TypeId::of::<V>(), TypeId::of::<E>());
        let cache = cache.get(&type_key)?;
        let cache = cache.as_any().downcast_ref::<CacheEntry<K, V, E>>()?;
        func(&cache.0)
    }

    fn use_cache_option_mut<K, V, E, F, R>(&self, func: F) -> Option<R>
    where
        K: 'static,
        V: 'static,
        E: 'static,
        ...
        R: 'static,
    {
        ...
        let cache = cache.as_any_mut().downcast_mut::<CacheEntry<K, V, E>>()?;
        func(&mut cache.0)
    }

    fn use_cache<K, V, E, R>(
        &self,
        func: impl FnOnce((Scope, &mut HashMap<K, Query<K, V, E>>)) -> R + 'static,
    ) -> R
    where
        K: Clone + 'static,
        V: Clone + 'static,
    ...
    fn get_or_create_query<K, V, E>(&self, key: K) -> (Query<K, V, E>, bool)
    where
        K: Clone + Eq + Hash + 'static,
        V: Clone + 'static,
        E: Clone + 'static,
    ...
    pub(crate) fn get_query_signal<K, V, E>(
        &self,
        cx: Scope,
        key: impl Fn() -> K + 'static,
    ) -> Signal<(Query<K, V, E>, bool)>
    where
        K: Hash + Eq + Clone + 'static,
        V: Clone + 'static,
        E: Clone + 'static,
    ...
    pub(crate) fn evict_and_notify<K, V, E>(&self, key: &K) -> Option<Query<K, V, E>>
    where
        K: Hash + Eq + 'static,
        V: 'static,
    {
